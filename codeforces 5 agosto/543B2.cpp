    ...
    if(dp[s0][t0]<=l0 and dp[s1][t1]<=l1)
    {
        ...
        for(int i=1;i<=n;i++){
            for(int j=1;j<=n;j++){
                ...
                if(dp[s0][i]+dp[i][j]+dp[j][t0]<=l0 and dp[s1][i]+dp[i][j]+dp[j][t1]<=l1 and dp[s0][i]!=-1 and dp[i][j]!=-1 and dp[j][t0]!=-1 and dp[s1][i]!=-1 and dp[j][t1]!=-1){
